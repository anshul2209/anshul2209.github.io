...
<nav class="{ isVisible ? 'visible': 'hidden'}" on:click={handleTabSelect} on:keydown={(e) => e.key === 'Enter' && handleTabSelect(e)} role="navigation" aria-label="Mobile navigation">
	<div class="logo">
		<h2 class="name">Anshul Bansal</h2>
		<h4 class="title">Full Stack Developer</h4>
	</div>
	<ul on:click={handleNavigation} on:keydown={(e) => e.key === 'Enter' && handleNavigation(e)} role="menu" aria-label="Navigation menu">
...
<div class="nav_bar" style="z-index: {isVisible ? 1 : 2}">
	<i class="fa fa-bars" on:click={handleToggle} on:keydown={(e) => e.key === 'Enter' && handleToggle()} role="button" tabindex="0" aria-label="Toggle navigation menu"></i>
